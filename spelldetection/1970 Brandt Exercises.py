...
def hyphenate_w(word):
    ...
    if len(suffixed_word) > 1:
        print('Suffixes encountered in word! The following suffixes were found: '
              + str(suffixed_word[1:]))
    ...
    if loops < 1:
        print('Finished! No syllables to process (besides possible '
              + 'suffixes. Returning output word')
        return vowel_split('-'.join(suffixed_word))

    # Print summary
    print('Characters to be processed (excluding prefixes/suffixes): \n- '
          + str(split_word))
    print('Total syllable count (excluding prefixes/suffixes): \n- '
          + str(loops+1))
    ...
    for pair in range(loops):
        ...
        print('Currently processing word-section number ' + str(pair+1) +
              '. Section analyzed is \n- ' + str(split_word[starting_point:]))
        ...
        if split_word[first_vowel+starting_point] in aai_group:
            ...
            print('aai-exception detected.', end='')
        ...
        # First, check to see if VV combination, if so process
            ...
            processed = process_two_length(split_word[starting_point:],
                                           ...
                                           second_vowel,
                                           vowel_two_end)
            print('VV combination detected!', end='')
            word_output = word_output + processed
        ...
        # After that, check for VCV combination if present
            ...
            print('VCV combination detected!', end='')
            word_output = word_output + processed
        ...
        # Check for 5-length+ combinations (VCCCV, VCCCCV, e.g. kasplant)
            ...
            processed = process_five_length(split_word[starting_point:],
                                            ...
                                            second_vowel,
                                            5)
            print('VCCCV combination detected!', end='')
            word_output = word_output + processed
        ...
        # Misc combinations. Will either be VCCCV or VCCV
            ...
            processed = process_misc_length(split_word[starting_point:],
                                            first_vowel,
                                            second_vowel)
            print('VCC(C)V combination detected!', end='')
            word_output = word_output + processed
...
# List containing all words mentioned in Brandt Corstius article
...
for word in range(len(df.iloc[:,0])):
    print(df.iloc[word,0])
    try:
        df.iloc[word,1] = ''.join(hyphenate_w(df.iloc[word,0]))
    except:
        pass

for word in range(len(df.iloc[:,0])):
    print(df.iloc[word,1],end='\n')
